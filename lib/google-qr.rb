...
class GoogleQR
  attr_accessor :data, :size, :use_https, :encoding, :error_correction, :margin, :style, :classname
  ...
  def render
    ...
    if self.classname
      classname = "class='" + self.classname + "' "
    else
      classname = nil
    end
    ...
    "<img src='#{self.to_s}'#{dimensions}#{classname}#{style}/>"
  end
